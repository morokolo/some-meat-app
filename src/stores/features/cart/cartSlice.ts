...
export const addToCart = createAsyncThunk(
  'cart/add',
  async (
    payload: { cartId: number; productId: number; quantity: number },
    { rejectWithValue }
  ) => {
    try {
...
export const removeFromCart = createAsyncThunk(
  'cart/remove',
  async (
    payload: { cartId: number; productId: number },
    { rejectWithValue }
  ) => {
    try {
      return await cartApi.removeFromCart(payload.cartId, payload.productId);
    } catch (e: any) {
      return rejectWithValue(e.message || 'Unknown error');
    }
  }
